---
title: Ear Training Basics
slug: ear_training_basics
difficulty: intermediate
tags: [song-analysis, ear-training, intervals]
prerequisites: [major_or_minor, intervals]
module: song-analysis
position: 8
summary: Practical ear training for guitarists — learning to identify intervals, chord quality, and tonal centers by sound rather than by counting.
---

## Why Ear Training Matters for Analysis

All the theory in this track is only useful if you can apply it to *listening*. The fastest song analysis is done by ear — hearing the key, the quality, and the chord functions without needing to pause and calculate. Ear training bridges the gap between knowing the theory and hearing it.

This lesson covers the practical ear skills most useful for song analysis: interval recognition, chord quality identification, and tonal center detection.

## Interval Recognition

An interval is the distance between two notes. Recognizing intervals by ear allows you to identify chord types, melodic movements, and the characteristic tones of scales and modes.

**Method: reference song associations.** Each interval has a well-known song that opens with it. Memorize one song per interval.

| Interval | Sound quality | Reference song (ascending) |
|----------|--------------|---------------------------|
| Minor 2nd (b2) | Tense, half-step | *Jaws* theme |
| Major 2nd | Neutral, stepwise | *Happy Birthday* (first two notes) |
| Minor 3rd (b3) | Minor, slightly dark | *Smoke on the Water* (opening riff) |
| Major 3rd | Bright, major | *When the Saints Go Marching In* |
| Perfect 4th | Open, strong | *Here Comes the Bride* |
| Tritone (#4/b5) | Dissonant, unstable | *The Simpsons* theme |
| Perfect 5th | Powerful, stable | *Star Wars* main theme |
| Minor 6th | Mournful | *The Entertainer* (in reverse) |
| Major 6th | Warm | *My Bonnie Lies Over the Ocean* |
| Minor 7th (b7) | Bluesy, unresolved | *Somewhere* (*West Side Story*, descending) |
| Major 7th | Bright, tense leading-tone | *Take On Me* (A-ha, melodic movement) |
| Octave | Resolved, same pitch | *Somewhere Over the Rainbow* |

## Chord Quality by Ear

The three essential chord qualities to identify by ear:

**Major chord:** Bright, stable, settled. The 3rd is a major 3rd above the root (4 semitones).

**Minor chord:** Darker, sadder, also stable. The 3rd is a minor 3rd above the root (3 semitones). The only difference from major is this one semitone — train your ear to hear it.

**Dominant 7th:** Major chord with a bluesy, unresolved quality — the b7 adds tension. Sounds like it wants to resolve to the chord a 4th above it.

**Diminished:** Very tense, hollow, symmetrical — equal-spaced minor 3rds. Relatively rare in pop.

**Training exercise:** Play random major and minor chords on your guitar and quiz yourself without looking. Then add dominant 7ths. Practice this until the quality distinction is immediate.

## Hearing the Tonal Center

The tonal center is the note that everything gravitates toward. Two exercises:

**Drone method:** Pick a drone note (open A string is convenient). Play the 12 notes of the chromatic scale over it one at a time. Some will sound consonant (they belong to the key centered on A). Others will sound dissonant. The seven consonant notes form the scale of the key.

**Resolution method:** Listen to a song and hum the note that feels most stable. Find that note on the guitar. That is the key note. Then determine whether it is the 1st, 3rd, or 5th of a major or minor chord — this tells you the root and quality of the tonic.

## Relative Pitch vs Absolute Pitch

Absolute pitch (perfect pitch) — the ability to identify any note by ear without a reference — is rare and is not required for song analysis. What you need is *relative pitch*: the ability to identify intervals and chord qualities in relation to a given reference note.

Relative pitch is trainable. The exercises above develop it systematically. With six months of consistent practice (10–15 minutes daily), most guitarists can identify common intervals and chord qualities reliably by ear.

## Putting Ear Training Into the Analysis Workflow

When you sit down to analyze a song:

1. Listen and identify the tonal center by feel (drone method if unsure).
2. Identify major or minor quality of the tonic chord (chord quality by ear).
3. Identify the first non-tonic chord — what interval is the root above the tonic? That interval pattern tells you the progression family.
4. Confirm by playing along and checking your analysis against the recording.

The theory in this track and the ear training in this lesson are two sides of the same skill. Theory tells you what to listen for; ear training trains you to hear it.

## What's Next

Ear training bridges theory and perception. The final lesson of the track — and the curriculum — walks through three complete song analyses from start to finish, applying every step of the workflow to real music and showing how the framework becomes automatic with consistent practice.
