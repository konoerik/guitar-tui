---
title: 12-Bar Blues in A
slug: twelve_bar_blues_a
difficulty: beginner
tags: [chords, progressions, blues, major-chords]
prerequisites: [a_d_e_progression]
module: first-progressions
position: 4
summary: Learn the 12-bar blues form in A — the structure behind blues, rock and roll, and countless jazz standards.
---

## What Is the 12-Bar Blues?

The 12-bar blues is a chord progression that repeats in a fixed 12-measure pattern. It is the structural backbone of the blues genre and a direct ancestor of rock and roll. The same pattern — I for four bars, IV for two, I for two, V for one, IV for one, I for two — appears in thousands of songs in every key.

In the key of A, the three chords are A (I), D (IV), and E (V).

## The Standard Pattern

The classic 12-bar form:

| Bar | Chord | Beats |
|-----|-------|-------|
| 1 | A | 4 |
| 2 | A | 4 |
| 3 | A | 4 |
| 4 | A | 4 |
| 5 | D | 4 |
| 6 | D | 4 |
| 7 | A | 4 |
| 8 | A | 4 |
| 9 | E | 4 |
| 10 | D | 4 |
| 11 | A | 4 |
| 12 | E | 4 |

Bar 12 (the *turnaround*) uses E to create tension that pulls back to A at the top of the next repeat.

## The Chords

### A Major

```diagram
type: chord
title: A Major
frets: [null, 0, 2, 2, 2, 0]
fingers: [null, null, 1, 2, 3, null]
```

### D Major

```diagram
type: chord
title: D Major
frets: [null, null, 0, 2, 3, 2]
fingers: [null, null, null, 1, 3, 2]
```

### E Major

```diagram
type: chord
title: E Major
frets: [0, 2, 2, 1, 0, 0]
fingers: [null, 2, 3, 1, null, null]
```

## Reading the Full Form

The tab below shows all 12 measures — one chord strum per bar, using duration 4 for four beats each. Play through it slowly before attempting it at tempo:

```diagram
type: tab
title: 12-Bar Blues in A
time: "4/4"
lines:
  - measures:
      - beats:
          - notes: [null, 0, 2, 2, 2, 0]
            label: "A"
            duration: 4
      - beats:
          - notes: [null, 0, 2, 2, 2, 0]
            label: "A"
            duration: 4
      - beats:
          - notes: [null, 0, 2, 2, 2, 0]
            label: "A"
            duration: 4
      - beats:
          - notes: [null, 0, 2, 2, 2, 0]
            label: "A"
            duration: 4
  - measures:
      - beats:
          - notes: [null, null, 0, 2, 3, 2]
            label: "D"
            duration: 4
      - beats:
          - notes: [null, null, 0, 2, 3, 2]
            label: "D"
            duration: 4
      - beats:
          - notes: [null, 0, 2, 2, 2, 0]
            label: "A"
            duration: 4
      - beats:
          - notes: [null, 0, 2, 2, 2, 0]
            label: "A"
            duration: 4
  - measures:
      - beats:
          - notes: [0, 2, 2, 1, 0, 0]
            label: "E"
            duration: 4
      - beats:
          - notes: [null, null, 0, 2, 3, 2]
            label: "D"
            duration: 4
      - beats:
          - notes: [null, 0, 2, 2, 2, 0]
            label: "A"
            duration: 4
      - beats:
          - notes: [0, 2, 2, 1, 0, 0]
            label: "E"
            duration: 4
```

## The Quick-Change Variation

A common variation adds D in the second bar, giving bars 1–2 a I–IV–I–I feel instead of four straight bars of I:

| Bar | Standard | Quick-Change |
|-----|----------|--------------|
| 1 | A | A |
| 2 | A | **D** |
| 3 | A | A |
| 4 | A | A |

Both forms are standard. The quick-change version has more harmonic movement early in the form and is common in uptempo blues and shuffle styles.

## Practising the Form

1. **Count while you play.** The 12-bar pattern is easy to lose track of in the heat of playing. Count "one two three four" aloud for each bar and say the chord name on beat 1 of each change.
2. **Loop without stopping.** The form repeats — when you reach bar 12, bar 1 follows immediately. Practise the transition from bar 12 (E) back to bar 1 (A) as a smooth loop.
3. **Record yourself.** Even a phone recording lets you hear whether your chord changes land on the beat, which is easy to miss when you are focused on the shapes.

## Why This Matters

The 12-bar blues is a musical grammar. Once the form is in your ears and hands, you will recognise it everywhere — in BB King, Chuck Berry, Eric Clapton, Led Zeppelin, and beyond. It is also the direct framework for the minor pentatonic scale you will learn in the pentatonic track: position 1 of the A minor pentatonic sits perfectly over this progression and is how most rock guitarists approach their first improvised solo.
